...
import { db, dataPath, uiDocPath } from "./firebase-config.js";
...
// DOM READY
// ─────────────────────────────────────────────────────────────────────────────
    // Scroll-reveal observer with ultra-robust viewport matching
    const observer = new IntersectionObserver(entries => {
        entries.forEach(e => { if (e.isIntersecting) e.target.classList.add('active'); });
    }, { threshold: 0.01, rootMargin: '100px 0px' });
    document.querySelectorAll('.reveal-up').forEach(el => observer.observe(el));

    // Failsafe: ensure all elements in the initial viewport show even on small/split screens
    setTimeout(() => {
...
function startDataFetch() {
    ...
    onSnapshot(uiDocPath('behold'), async snap => {
        ...
        if (!snap.exists() || !snap.data().url) {
            ...
            window.beholdGalleryItems = [];
            if (window._renderGalleryCombined) window._renderGalleryCombined();
            return;
        }
        const url = snap.data().url;
        ...
        try {
            ...
            window.beholdGalleryItems = posts.map(p => ({
                id: p.id,
            ...
            console.warn('[main] Behold fetch failed:', e);
        }
    }, err => console.warn('[main] behold fetch failed:', err));
    ...
    function syncCollection(collName, defaults, renderFn) {
        onSnapshot(dataPath(collName), snap => {
            ...
            // Refresh contextual list if CMS is active
...
function renderGallery(items) {
    if (items) manualGalleryItems = items;
    const beholdItems = window.beholdGalleryItems || [];
    currentGalleryItems = [...beholdItems, ...manualGalleryItems];
...
window._renderGalleryCombined = renderGallery;
